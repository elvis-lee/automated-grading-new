...
int Enqueue(struct Queue *que, const uint8_t *data, uint16_t len)
{
	int i;
	for (i = 0; !QueueFull(que) && (i < len); i++)
	{
		que->q[que->pWR] = data[i];
		que->pWR = ((que->pWR + 1) ==  QUEUE_SIZE) ? 0 : que->pWR + 1;
	}
	if (!TxPrimed)
	{
		TxPrimed = 1;
...
int Dequeue(struct Queue *que, uint8_t *data, uint16_t len)
{
	int i;
	for (i = 0; !QueueEmpty(que) && (i < len); i++)
	{
		data[i] = que->q[que->pRD];
		que->pRD = ((que->pRD + 1) ==  QUEUE_SIZE) ? 0 : que->pRD + 1;
	}
	return i;
}
...
{   
    uint32_t num;
    flag_mutex = 1;
    num = (q->pWR >= q->pRD) ? q->pWR - q->pRD : q->pWR + QUEUE_SIZE - q->pRD;
    flag_mutex = 0;
	return num;
